$(document).ready(function() {
  ...
  $("button").click(function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        for (var i = 0; i < data.results.length; i++) {
            $("#incumbentView").append('<aside class="' + data.results[i].party + '">' + data.results[i].name + ' (' + data.results[i].party + ')</aside>');
    ...
    //     for (var i = 0; i < data.results.length; i++) {
    //       $("#challengerView").append('<p id="' + data.results[i].candidate_id + '">' + data.results[i].name + ' (' + data.results[i].party + ')</p>');
    //
    //       // Use candidate_id to retrieve that candidate's committee_ids
    //       $.ajax({
    //         url: "https://api.open.fec.gov/v1/candidate/" + data.results[i].candidate_id + "/committees/?sort=name&per_page=100&page=1&api_key=" + apiKey,
    ...
    //         success: function(moreData) {
    //           for (var i = 0; i < moreData.results.length; i++) {
    //             if (data.results[i].party === "REP") {
    //               $("#challengerView").append('<aside class="' + data.results[i].party + '">' + data.results[i].name + ' (' + data.results[i].party + ')</aside>');
    //             } else if (data.results[i].party === "DEM") {
    //               $("#challengerView").append('<aside class="' + data.results[i].party + '">' + data.results[i].name + ' (' + data.results[i].party + ')</aside>');
    //             } else {
    //               $("#challengerView").append('<aside class="' + data.results[i].party + '">' + data.results[i].name + ' (' + data.results[i].party + ')</aside>');
    //             }
